...
class FilesCount extends BaseWidget
{
	...
	public function init()
	{
		switch(1)
		{
			case !($this->model instanceof FilesModel) && (($this->parentType == null) || ($this->parentId == null)):
			$this->model = null;
			break;
			
			default:
			$this->model = ($this->model instanceof FilesModel) ? $this->model : FilesModel::findModel([$this->parentId, $this->parentType]);
			break;
		}	
		parent::init();
	}
	...
	public function run()
	{
		...
		$this->options['href'] = \Yii::$app->urlManager->createUrl(['/files/index/'.$this->parentType."/".$this->parentId, '__format' => 'modal']);
		$this->options['title'] = \Yii::t('yii', 'View Files');
